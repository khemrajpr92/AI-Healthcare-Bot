...
def predict_symptom(user_input, symptom_list):
    ...
    for symptom in symptom_list:
        ...
        count_vector = np.zeros(
            (2, len(set(user_input_tokens + symptom_tokens))))
    ...
    if request.args["message"] == "undefined":
        ...
        if currentState == 2:
            ...
            if '2' in user_message.lower() or 'check' in user_message.lower():
                ...
                username = all_result['name']
                response.append(
                    username+", What symptoms are you experiencing?")
                response.append(
                    '<a href="/diseases" target="_blank">Symptoms List</a>')
                userSession[sessionId] = userSession.get(sessionId) + 1
        ...
        if currentState == 4:
            ...
            if '1' in user_message or 'disease' in user_message:
                disease, type = predict_disease_from_symptom(
                    all_result['symptoms'])
                response.append(
                ...
                username = all_result['name']
                response.append(
                    username+", Could you describe the symptoms you're suffering from?")
                response.append("1. Check Disease")
                response.append(
                    '<a href="/diseases" target="_blank">Symptoms List</a>')
                userSession[sessionId] = userSession.get(sessionId) + 1
        ...
        if currentState == 5:
            if '1' in user_message or 'disease' in user_message:
                ...
                username = all_result['name']
                response.append(
                    username+", What are the symptoms that you're currently dealing with?")
                response.append("1. Check Disease")
                response.append(
                    '<a href="/diseases" target="_blank">Symptoms List</a>')
                userSession[sessionId] = userSession.get(sessionId) + 1
        ...
        if currentState == 6:
            ...
            if '1' in user_message or 'disease' in user_message:
                disease, type = predict_disease_from_symptom(
                    all_result['symptoms'])
                response.append(
                    "The following disease may be causing your discomfort")
                response.append(disease)
                response.append(
                    f'<a href="https://www.google.com/search?q={type} disease hospital near me" target="_blank">Search Near By Hospitals</a>')
                userSession[sessionId] = 10
        ...
        if currentState == 10:
            response.append(
                '<a href="/user" target="_blank">Predict Again</a>')
